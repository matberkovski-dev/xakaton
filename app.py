...
# На Render и других PaaS используем /tmp для записи;
# локально — папку instance рядом с проектом
if os.environ.get('RENDER') or os.environ.get('DYNO'):
    DATA_DIR   = '/tmp/quest_data'
else:
    DATA_DIR   = os.path.join(BASE_DIR, 'instance')
DB_PATH    = os.path.join(DATA_DIR, 'quest.db')
...
app.secret_key = os.environ.get('SECRET_KEY', 'eng_quest_2024_secret_dev')
...
# Инициализация при старте (работает и с gunicorn)
