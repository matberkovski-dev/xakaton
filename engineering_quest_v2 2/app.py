...
def public_settings():
    """Public settings any client can read (no auth required)."""
    with db() as c:
        interval = c.execute("SELECT value FROM Settings WHERE key='refresh_interval_s'").fetchone()
    return jsonify({'ok': True, 'refresh_interval_s': int(interval[0]) if interval else 10})
...
def game_reset():
    ...
    with db() as c:
        ...
        c.execute("UPDATE Teams SET Spent=0, Bridge_Weight=0.0, Quantity_Assignments=0, Start_Budget=500, Status='Не приступили к заданию' WHERE Role='Участник'")
